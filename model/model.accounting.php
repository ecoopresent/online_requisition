...
class Accounting extends DBHandler {
    ...
    {
        $query = "SELECT id, pr_id, department, payee, date_prepared, date_needed, particulars, amount, purpose, remarks, charge_to, budget, liquidated_on, prepared_by, department_head, president, accounting, cash_status FROM cashcheck WHERE cash_status = 'Finished' OR cash_status = 'Checked'";
        $stmt = $this->prepareQuery($this->conn, $query);
        return $this->fetchAssoc($stmt);
    }

    public function UpdateStatus($id,$cash_status)
    {
        if($cash_status=="Disapproved2" || $cash_status=="Approved"){
            $full_name = "";
        }
        $query = "UPDATE cashcheck SET cash_status = '$cash_status' WHERE id = '$id'";
        $stmt = $this->prepareQuery($this->conn, $query);
