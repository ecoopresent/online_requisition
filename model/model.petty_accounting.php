...
class PettyAccounting extends DBHandler {
    ...
    public function getPettycashlist($status,$user_dept)
    {
        ...
        $query = "SELECT id, department, voucher_date, voucher_no, particulars, cash_advance, actual_amount, charge_to, liquidated_on, requested_by, approved_by, authorized, pettycash_status FROM pettycash WHERE pettycash_status != 'Approved1' AND pettycash_status != 'Finished' AND pettycash_status != '' AND pettycash_status != 'Disapproved1' AND pettycash_status != 'Disapproved' AND payee_id = '$payee_id'";
    ...
    public function declinePetty($id)
    {
        $query = "UPDATE pettycash SET approved_by = '.', authorized = '.', pettycash_status = 'Disapproved' WHERE id = '$id'";
        $stmt = $this->prepareQuery($this->conn, $query);
        $this->execute($stmt);

        $query = "UPDATE liquidation SET checked_by = '.', approved_by = '.' WHERE pettycash_id = '$id'";
        $stmt = $this->prepareQuery($this->conn, $query);
        return $this->execute($stmt);
    }
