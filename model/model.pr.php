...
class Pr extends DBHandler {
    ...
    public function submitPRTrade($pr_id,$name_Approver){
        $pr_status = "Approved";
        $approver = $name_Approver;
        $date_today = date('Y-m-d');
        $query = "UPDATE pr SET pr_status = '$pr_status', approved_by = '$approver', date_approve = '$date_today' WHERE id = '$pr_id'";
        $stmt = $this->prepareQuery($this->conn, $query);
        return $this->execute($stmt);
    }
    ...
    {
        $query = "SELECT COUNT(id) FROM pr ORDER BY id DESC LIMIT 1";
        $stmt = $this->prepareQuery($this->conn, $query);
        $row = $this->fetchRow($stmt);
        $id = $row[0];
        // if($id==""){
    ...
    public function checkCStatusOperation($id){
