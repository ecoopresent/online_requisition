...
switch($mode) {
    ...
    case "table";
        ...
        foreach($canvasing as $k=>$v) {
            ...
            $v['price1'] = $v['price1'] / $v['qty'];
            $v['price2'] = $v['price2'] / $v['qty'];
            $v['price3'] = $v['price3'] / $v['qty'];
            $v['price4'] = $v['price4'] / $v['qty'];
            $v['price5'] = $v['price5'] / $v['qty'];
    ...
    case "UpdateCanvasPre";
        $id = Sanitizer::filter('id', 'post');
        $canvas_status = Sanitizer::filter('canvas_status', 'post');
        $remarks = Sanitizer::filter('remarks', 'post');
        $approver = Sanitizer::filter('approver', 'post');
        $canvasing->Update_CanvasPre($id,$canvas_status,$remarks,$approver);
