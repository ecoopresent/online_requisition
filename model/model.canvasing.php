...
class Canvasing extends DBHandler {
    ...
    public function Update_CanvasPre($id,$canvas_status,$remarks,$approver)
    {  
    ...
    {  
        ...
        if($canvas_status=="Rejected"){
            $query = "UPDATE canvas SET operation_incharge = '-', remarks = '$remarks' WHERE pr_id = '$id'";
            $stmt = $this->prepareQuery($this->conn, $query);
            return $this->execute($stmt);
        }else if($canvas_status=="Rejected1"){
            $query = "UPDATE canvas SET approved_by = '-', remarks = '$remarks' WHERE pr_id = '$id'";
            $stmt = $this->prepareQuery($this->conn, $query);
            return $this->execute($stmt);
        }else{

            if($approver=="Jerome T. Chua"){
                $query = "UPDATE canvas SET operation_incharge = '$approver',remarks = '$remarks',date_approved = '$date_today' WHERE pr_id = '$id'";
                $stmt = $this->prepareQuery($this->conn, $query);
                return $this->execute($stmt);
            }else{
