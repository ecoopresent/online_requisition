...
function resendRCA(id,approver_type){
    var wow = [id,approver_type];
    confirmed("save",resendRCA_callback, "Do you really want to resend this?", "Yes", "No", wow);
}

function resendRCA_callback(wow){
    var id = wow[0];
    var at = wow[1];
    toggleLoad();
    window.location.href="tcpdf/examples/rca_finalResend.php?id="+id+"&at="+at;
}
