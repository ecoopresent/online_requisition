...
switch($mode) {
	...
	case "table";
        ...
        foreach($approvedpr as $k=>$v) {
            ...
            if($v['pr_status'] == 'Rejected'){
                ...
                $approvedpr[$k]['action'] = "<button class='btn btn-success btn-sm'  onclick='resend_Canvas(".$v['id'].")'><i class='fas fa-sm fa-paper-plane'></i> Re-send</button>
                                        <button class='btn btn-danger btn-sm' onclick='view_PR(".$v['id'].")'><i class='fas fa-sm fa-eye'></i> View PR & Canvassed</button>";
            }else if($v['pr_status'] == "FinalFinished"){
                $approvedpr[$k]['action'] = "<button class='btn btn-danger btn-sm' onclick='view_PR(".$v['id'].")'><i class='fas fa-sm fa-eye'></i> View PR & Canvassed</button> <button class='btn btn-secondary btn-sm' onclick='undoStatus_PR(".$v['id'].")'><i class='fas fa-sm fa-undo'></i> Undo</button>";
            }else if($v['pr_status'] == "Finished" && $v['cash_status'] == "PreApproved"){
                $approvedpr[$k]['action'] = "<button class='btn btn-danger btn-sm' onclick='view_PR(".$v['id'].")'><i class='fas fa-sm fa-eye'></i> View PR</button> <button class='btn btn-primary btn-sm' onclick='view_RCA(".$v['idRCA'].")'><i class='fas fa-sm fa-eye'></i> View RCA</button> <button class='btn btn-success btn-sm' onclick='resendRCA(".$v['idRCA'].",\"".$v['approver_type']."\")'><i class='fas fa-sm fa-paper-plane'></i> Resend</button>";
            }else{
                ...
                $pr_type = $v['pr_type'];
                if($pr_type=="local" || $pr_type=="import"){
                    $approvedpr[$k]['action'] = "<button class='btn btn-primary btn-sm' onclick='open_PR(".$v['id'].",".$id.",\"".$department."\",\"".$payee."\",\"".$date_prepared."\",\"".$date_needed."\",\"".$particulars."\",\"".$amount."\",\"".$purpose."\",\"".$remarks."\",\"".$charge_to."\",\"".$budget."\",\"".$liquidated_on."\",\"".$label."\")'><i class='fas fa-sm fa-mouse-pointer'></i> Request Cash Advance</button>
                                        <button class='btn btn-danger btn-sm' onclick='view_PR(".$v['id'].")'><i class='fas fa-sm fa-eye'></i> View PR & Canvassed</button> <button class='btn btn-success btn-sm' onclick='changeStatus_PR(".$v['id'].")'><i class='fas fa-sm fa-check-double'></i> Finish</button>";
                }
            }
    ...
    case "changeStatus";
        ...
        $approvedpr->changeStatus($id,$status);
        $response = array("code"=>1, "message"=>"Change Status Saved");
        break;
    ...
    case "extrarespond";
        ...
        $approvedpr->extrarespondTo($id,$remarks,$approver,$cash_status);
        $response = array('code'=>1, "message"=> "Respond Success");
        break;
