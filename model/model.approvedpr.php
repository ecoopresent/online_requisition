...
class Approvedpr extends DBHandler {
    ...
    {   
        ...
        // else if($canvas_status=="Submitted"){
        //     $WHERE = "a.pr_status = 'Finished' ORDER BY c.date_approved ASC";
        // }
        $WHERE = "";
    ...
    {
        $query = "UPDATE pr SET pr_status = '$status' WHERE id = '$id'";
        $stmt = $this->prepareQuery($this->conn, $query);
    ...
    public function countCanvasrejected($status){
        $query = "SELECT COUNT(id) FROM pr WHERE pr_status = '$status' ";
        $stmt = $this->prepareQuery($this->conn, $query);
        $row = $this->fetchRow($stmt);
        return $row[0];
    }
    ...
    public function checkStatusRCAextra($id){
        
        $query = "SELECT head_approver FROM cashcheck WHERE id = '$id'";
        $stmt = $this->prepareQuery($this->conn, $query);
        $row = $this->fetchRow($stmt);
        return $row[0];
    }
    ...
    public function extrarespondTo($id,$remarks,$approver,$cash_status)
    {   
        $date_today = date('Y-m-d');
        $query = "UPDATE cashcheck SET cash_status = '$cash_status', head_approver = '$approver', remarks = '$remarks', date_headapproved = '$date_today' WHERE id = '$id'";
        $stmt = $this->prepareQuery($this->conn, $query);
        return $this->execute($stmt);
    }
